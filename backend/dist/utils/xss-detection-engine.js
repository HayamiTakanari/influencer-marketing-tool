...
exports.XSS_PATTERNS = exports.DANGEROUS_ATTRIBUTES = exports.DANGEROUS_TAGS = exports.ALLOWED_ATTRIBUTES = exports.ALLOWED_TAGS = exports.XSSRiskLevel = void 0;
exports.detectXSSAttack = detectXSSAttack;
...
exports.DANGEROUS_TAGS = DANGEROUS_TAGS;
// 危険な属性（完全ブラックリスト）
...
exports.DANGEROUS_ATTRIBUTES = DANGEROUS_ATTRIBUTES;
// 許可されるHTMLタグ（ホワイトリスト）
const ALLOWED_TAGS = {
    // 基本的なテキスト装飾
...
exports.ALLOWED_ATTRIBUTES = ALLOWED_ATTRIBUTES;
// XSS攻撃パターン（正規表現）
const XSS_PATTERNS = [
    // スクリプトタグ
 ...
 */
var XSSRiskLevel;
(function (XSSRiskLevel) {
    XSSRiskLevel["LOW"] = "low";
    XSSRiskLevel["MEDIUM"] = "medium";
    XSSRiskLevel["HIGH"] = "high";
    XSSRiskLevel["CRITICAL"] = "critical";
})(XSSRiskLevel || (exports.XSSRiskLevel = XSSRiskLevel = {}));
/**
 * メインのXSS検出関数
 */
async function detectXSSAttack(input, context = {}, userId, ipAddress) {
    const result = {
        isXSS: false,
 ...
 * 既知のペイロード検出
 */
function detectKnownPayloads(input) {
    const matches = [];
 ...
 * HTML構造解析
 */
function analyzeHtmlStructure(input, context) {
    const dangerousTags = [];
 ...
 * ヒューリスティック分析
 */
function performHeuristicAnalysis(input) {
    let score = 0;
    // JavaScriptキーワードの検出
 ...
 * 入力のクリーニング
 */
function cleanInput(input, context) {
    let cleaned = input;
 ...
 * XSS検出ログの記録
 */
async function logXSSDetection(input, result, userId, ipAddress) {
    try {
