...
const recommendInfluencersForProject = async (req, res) => {
    try {
        ...
        const { title, description, category, budget, targetPlatforms, brandName, productName, campaignObjective, campaignTarget, messageToConvey } = req.body;
        ...
        const recommendedInfluencers = influencers
            ...
            const connectedPlatforms = influencer.socialAccounts.map((acc) => acc.platform);
            const platformMatches = targetPlatforms.filter(platform => connectedPlatforms.includes(platform));
            ...
            if (influencer.categories && influencer.categories.length > 0) {
                const categoryMatch = influencer.categories.some((cat) => cat.toLowerCase().includes(category.toLowerCase()) ||
            ...
            const totalFollowers = influencer.socialAccounts.reduce((sum, acc) => sum + (acc.followerCount || 0), 0);
            ...
            const avgEngagementRate = influencer.socialAccounts.length > 0
                ? influencer.socialAccounts.reduce((sum, acc) => sum + (acc.engagementRate || 0), 0) /
            ...
            return {
                ...
                socialAccounts: influencer.socialAccounts.map((acc) => ({
                    id: acc.id,
