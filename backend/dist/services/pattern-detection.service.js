...
exports.patternDetectionService = exports.AttackType = void 0;
...
var AttackType;
(function (AttackType) {
    AttackType["SQL_INJECTION"] = "sql_injection";
    AttackType["XSS_ATTACK"] = "xss_attack";
    AttackType["COMMAND_INJECTION"] = "command_injection";
    AttackType["PATH_TRAVERSAL"] = "path_traversal";
    AttackType["XXE_ATTACK"] = "xxe_attack";
    AttackType["LDAP_INJECTION"] = "ldap_injection";
    AttackType["NOSQL_INJECTION"] = "nosql_injection";
    AttackType["TEMPLATE_INJECTION"] = "template_injection";
    AttackType["DESERIALIZATION_ATTACK"] = "deserialization_attack";
    AttackType["FILE_INCLUSION"] = "file_inclusion";
    AttackType["SSRF_ATTACK"] = "ssrf_attack";
    AttackType["CSRF_ATTACK"] = "csrf_attack";
    AttackType["CLICKJACKING"] = "clickjacking";
    AttackType["HTTP_POLLUTION"] = "http_pollution";
    AttackType["PROTOCOL_CONFUSION"] = "protocol_confusion";
    AttackType["BUFFER_OVERFLOW"] = "buffer_overflow";
    AttackType["FORMAT_STRING"] = "format_string";
    AttackType["RACE_CONDITION"] = "race_condition";
    AttackType["TIME_BASED_ATTACK"] = "time_based_attack";
...
class PatternDetectionService {
     ...
     * 個別パターンルールのチェック
     */
    async checkPatternRule(logData, rule) {
     ...
     * 行動ベース検知
     */
    async runBehaviorDetection(logData) {
        const detections = [];
     ...
     * 時系列異常検知
     */
    async runTimeSeriesDetection(logData) {
     ...
     * アンサンブル検知
     */
    async runEnsembleDetection(logData, previousDetections) {
    ...
    getTargetData(targets, context) {
        return targets[context] || targets['all'];
    }
    calculateRiskScore(rule, score, matches) {
    ...
    generateRecommendations(rule, matches) {
    ...
    async analyzeBehaviorIndicators(logData, pattern) {
        const indicators = [];
    ...
    detectTimeSeriesAnomalies(history) {
    ...
    // ヘルパーメソッド
    async calculateRequestFrequency(ipAddress, windowMinutes) {
        // 実装簡略化
        return Math.random() * 100;
    }
    async calculateErrorRate(ipAddress, windowMinutes) {
        // 実装簡略化
    ...
    updatePatternRule(ruleId, updates) {
