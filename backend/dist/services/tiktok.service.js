...
class TikTokService {
    ...
    async getVideoInfo(videoUrl) {
        try {
            this.ensureApiKey();
            const response = await fetch(`${this.baseUrl}/?url=${encodeURIComponent(videoUrl)}&hd=1`, {
                method: 'GET',
                headers: {
     ...
     */
    extractUsernameFromUrl(videoUrl) {
        try {
     ...
     */
    calculateEngagementRate(videoData) {
        try {
            const { play_count = 0, digg_count = 0, comment_count = 0, } = videoData;
    ...
    formatVideoData(rawData) {
        const { aweme_id, title, cover, origin_cover, play, wmplay, music, play_count, digg_count, comment_count, create_time, author, region, } = rawData;
     ...
     */
    async getVideoStatistics(videoUrl) {
        try {
            const videoData = await this.getVideoInfo(videoUrl);
            return {
     ...
     */
    async getUserVideosStats(username, maxVideos = 10) {
        try {
            this.ensureApiKey();
     ...
     * @param username - TikTok ユーザー名
     */
    async getUserFollowerList(username) {
        try {
            this.ensureApiKey();
            // Return helpful message since RapidAPI endpoint doesn't support username-based queries
            return {
    ...
    async searchVideos(keyword, maxResults = 10) {
        try {
            this.ensureApiKey();
            const response = await fetch(`${this.baseUrl}?keywords=${encodeURIComponent(keyword)}&count=${maxResults}`, {
                method: 'GET',
                headers: {
                    'x-rapidapi-key': this.apiKey,
                    'x-rapidapi-host': this.apiHost,
