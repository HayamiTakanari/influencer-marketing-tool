...
export class BillingService {
  ...
  static async createInvoice(projectId: string, companyId: string, influencerId: string, amount: number) {
  ...
  static async getPendingInvoices(userId: string, role: 'COMPANY' | 'INFLUENCER') {
    if (role === 'COMPANY') {
      const client = await prisma.company.findUnique({
        where: { userId },
      });
