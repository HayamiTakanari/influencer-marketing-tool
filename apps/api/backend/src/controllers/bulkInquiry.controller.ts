...
import { createBulkInquirySchema, updateInquiryResponseSchema } from '../schemas/bulkInquiry';
...
export const createBulkInquiry = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const responses = await Promise.all(
      validatedData.targetInfluencers.map(influencerId =>
...
export const getMyInquiryResponses = async (req: AuthRequest, res: Response) => {
  try {
    const { user } = req;
...
export const updateInquiryResponse = async (req: AuthRequest, res: Response) => {
  try {
    const { user } = req;
    const { id } = req.params;
    ...
    const validatedData = updateInquiryResponseSchema.parse(req.body);
    ...
    await prisma.notification.create({
      data: {
        userId: existingResponse.inquiry.company.userId,
...
export const getBulkInquiryById = async (req: AuthRequest, res: Response) => {
  try {
    const { user } = req;
    const { id } = req.params;
    ...
    if (user?.role === 'COMPANY' || user?.role === 'COMPANY') {
      ...
      if (!client || inquiry.companyId !== client.id) {
