 ...
 */
export const getVerificationDocumentStatus = async (userId: string, documentType?: DocumentType) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { role: true },
 ...
 * Chapter 1-2: 本人確認の承認
 */
export const approveVerificationDocument = async (
  documentId: string,
  adminId: string
): Promise<void> => {
  try {
 ...
 */
export const rejectVerificationDocument = async (
  documentId: string,
