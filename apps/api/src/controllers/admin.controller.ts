...
export const getProjectDetail = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      ...
      clientEmail: project.company.user.email,
...
export const getUsers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const formattedUsers = users.map(u => ({
      id: u.id,
      name: u.role === 'COMPANY' ? u.company?.companyName : u.role === 'INFLUENCER' ? u.influencer?.displayName : u.email,
      email: u.email,
      role: u.role,
