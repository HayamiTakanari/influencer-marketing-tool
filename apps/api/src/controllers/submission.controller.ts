...
const requestRevisionSchema = z.object({
  approvalNotes: z.string().min(1, 'Revision notes are required'),
});
...
export const getInfluencerSubmissions = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.userId;
    const userRole = (req as any).user.role;
...
export const approveSubmission = async (req: Request, res: Response) => {
  try {
    ...
    if (!submission || submission.companyId !== client.id) {
