...
import { FaTiktok, FaSync, FaTrash, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
const TikTokAccountManager: React.FC<TikTokAccountManagerProps> = ({
  socialAccountId,
  username,
  onRefresh,
}) => {
  ...
  const fetchAccountStats = async () => {
    ...
    try {
      setLoading(true);
      const response = await api.get(`/tiktok/account/${socialAccountId}/stats`);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const response = await api.post(`/tiktok/sync/${socialAccountId}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/tiktok/account/${socialAccountId}`);
