...
import { FaInstagram, FaSync, FaTrash, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
const InstagramAccountManager: React.FC<InstagramAccountManagerProps> = ({
  socialAccountId,
  username,
  onRefresh,
}) => {
  ...
  const fetchAccountStats = async () => {
    ...
    try {
      setLoading(true);
      const response = await api.get(`/instagram/account/${socialAccountId}/stats`);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const response = await api.post(`/instagram/sync/${socialAccountId}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/instagram/account/${socialAccountId}`);
