...
import { FaTiktok, FaCheckCircle, FaTimesCircle, FaSpinner } from 'react-icons/fa';
...
const TikTokAccountVerification: React.FC<TikTokVerificationProps> = ({
  ...
  const isValidTikTokUsername = (username: string): boolean => {
    // Username should be 2-24 characters, alphanumeric with dots and underscores
    const usernameRegex = /^[a-zA-Z0-9._]{2,24}$/;
    return usernameRegex.test(username.trim());
  };

  // Fetch user information from TikTok API by username
  const handleFetchUserInfo = async () => {
    try {
      ...
      if (!username.trim()) {
        setMessage({ type: 'error', text: 'TikTok ユーザー名を入力してください' });
        setLoading(false);
        return;
      }

      if (!isValidTikTokUsername(username)) {
        setMessage({ type: 'error', text: '無効なユーザー名です。2～24文字の英数字（. _ を含む）で入力してください' });
        setLoading(false);
        return;
      }

      // Fetch user info from backend
      const response = await api.get(`/tiktok/user/${username.trim()}`);
      ...
      if (response.data.success) {
        ...
        setMessage({
          type: 'info',
          text: `@${response.data.data.username} の TikTok プロフィール情報を取得しました`,
        });
      } else {
  ...
  // Verify and add TikTok account using username
  const handleVerifyAccount = async () => {
    try {
      ...
      const response = await api.post('/tiktok/verify-account', {
          ...
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                TikTok ユーザー名: @{userInfo.username}
              </p>
            </div>
          </div>
  ...
  return (
    <Card className="border-tiktok">
      <div className="flex items-center space-x-2 mb-6">
        <FaTiktok className="text-2xl text-black" />
        <h3 className="text-xl font-bold text-gray-800">TikTok アカウント認証</h3>
      </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              TikTok ユーザー名 <span className="text-red-500">*</span>
            </label>
            <p className="text-xs text-gray-500 mb-2">
              あなたの TikTok ユーザー名を入力してください。（例: tiktok、username など）
            </p>
...
export default TikTokAccountVerification;
